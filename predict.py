...
from networks.a_satmvsnet import A_SATMVSNet
...
parser.add_argument('--model', default="a-satmvsnet", help='select model', choices=["a-satmvsnet"])
...
parser.add_argument('--dataset_root', default='/file/.../', help='dataset root')
...
parser.add_argument('--ifShow', default=False, help='if show the depth map')
...
parser.add_argument('--lamb', type=float, default=1.5, help="lamb in a-satmvsnet")
parser.add_argument('--cr_base_chs', type=str, default="8,8,8", help='cost regularization base channels')
parser.add_argument('--gpu_id', type=str, default="0")
parser.add_argument('--savePfm', type=str, default="")
...
def predict():
    ...
    if args.model == "a-satmvsnet":
        model = A_SATMVSNet(lamb=args.lamb, stage_configs=[int(nd) for nd in args.ndepths.split(",") if nd],
                       base_chs=[int(ch) for ch in args.cr_base_chs.split(",") if ch],
                       geo_model=args.geo_model, use_qc=args.use_qc)
        print("===============> Model: A-SATMVSNet ===========>")
    else:
    ...
    output_folder = os.path.join(args.savePfm, 'mvs_results')
    ...
    with open('/file/sync_files/SatMVS-main/checkpoints/time_record.txt', 'w+') as f:
        f.write("final, time = {:3f}, test results = {}".format(time.time() - t0, avg_test_scalars.mean()))
