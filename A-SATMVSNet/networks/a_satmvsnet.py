...
def compute_depth(feats, proj_mats, reweight, depth_samps, cost_reg, lamb, geo_model, is_training=False, use_qc=False):
    '''
    ...
    for src_fea, src_proj in zip(src_feats, src_projs):
        ...
        # reweight
        rw = reweight(warped_volume)
        warped_volume = (rw + 1) * warped_volume
...
class A_SatMVSNet(nn.Module):
    def __init__(self, geo_model, lamb=1.5, stage_configs=[64, 32, 8], grad_method="detach", base_chs=[8, 8, 8],
        ...
        self.feature_extraction = FeatureNet1(base_channels=feat_ext_ch, num_stage=self.num_stage)
        self.reweight = nn.ModuleList([featVolumeAggModule(self.feature_extraction.out_channels[i]) for i in range(self.num_stage)])
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for stage_idx in range(self.num_stage):
            ...
            outputs_stage = compute_depth(features_stage, proj_matrices_stage,
                                          reweight = self.reweight[stage_idx],
